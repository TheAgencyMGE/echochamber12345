...
import { GolfRoom, Player } from '../../../shared/types/golf';
import { golfCourses } from '../../data/golfCourses';

interface GameHUDMultiplayerProps {
  room: GolfRoom;
  myPlayer: Player | null;
}

export const GameHUDMultiplayer: React.FC<GameHUDMultiplayerProps> = ({ room, myPlayer }) => {
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-green-300">
              {currentPlayer?.name} {currentPlayer?.id === myPlayer?.id && '(You)'}
            </p>
          </div>
        </div>
      </motion.div>
      ...
      >
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-white font-semibold">Hole {currentHole?.id || room.currentHole + 1}</h3>
      ...
      >
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-green-300">
              {myPlayer?.holeStrokes[room.currentHole] || 0} strokes this hole
            </p>
          </div>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {room.players.map((player: Player, index: number) => {
            const ball = room.balls.find(b => b.id === player.id);
                ...
                <div className="text-sm space-y-1">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className={`font-semibold ${
                      (player.holeStrokes[room.currentHole] || 0) <= (currentHole?.par || 3)
                        ? 'text-green-400' 
                        : 'text-yellow-400'
                    }`}>
                      {player.holeStrokes[room.currentHole] || 0}
                    </span>
                  </div>
                </div>
              </motion.div>
